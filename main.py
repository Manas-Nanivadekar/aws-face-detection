...
def usage_demo(url: Url):
    ...
    girl_image = RekognitionImage(
        {'Bytes': girl_image_response.content}, "reference-image", rekognition_client)
    group_image_response = requests.get(new_image)
    group_image = RekognitionImage(
        {'Bytes': group_image_response.content}, "new-image", rekognition_client)
